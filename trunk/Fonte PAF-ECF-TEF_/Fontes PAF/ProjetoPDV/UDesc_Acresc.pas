...
begin
     ...
     then begin
          Informa ('O Valor do Desconto/Acréscimo não pode ser Superior ao Valor do Cupom!');
...
begin
     ...
     then begin
          if Confirma('Confirma a Operação?')=mrYes
          then begin
               ...
               then begin
                     ...
                     then begin
                          Informa('Limite de Desconto Ultrapassado!!'+#13+
                                  'O Seu Limite Máximo é: '+FormatFloat('#####0.00',vPerc_Desc_Usuario)+' %');
                          ed_desconto.Value:=0;
                          ed_descontoPerc.Value:=0;
