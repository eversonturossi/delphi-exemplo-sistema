object FConfigECF: TFConfigECF
  Left = 366
  Top = 174
  Width = 608
  Height = 409
  BorderIcons = [biSystemMenu]
  Caption = 'Configura'#231#227'o e Informa'#231#245'es da Impressora Fiscal'
  Color = clInfoBk
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  Menu = MainMenu1
  OldCreateOrder = False
  Position = poDesktopCenter
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Bevel1: TBevel
    Left = 0
    Top = 0
    Width = 600
    Height = 336
    Align = alClient
    Shape = bsFrame
    Style = bsRaised
  end
  object BtnImprimir: TSpeedButton
    Left = 370
    Top = 304
    Width = 112
    Height = 30
    Cursor = crHandPoint
    Caption = '&Imprimir'
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      5353535353532333322C2C24241E255353535353535353535353}
    ParentFont = False
    OnClick = BtnImprimirClick
  end
  object BtFechar: TBitBtn
    Left = 485
    Top = 304
    Width = 112
    Height = 28
    Cursor = crHandPoint
    Caption = 'Esc - Fechar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    OnClick = BtFecharClick
    Glyph.Data = {
      
      FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
  end
  object GroupBox1: TGroupBox
    Left = 3
    Top = 3
    Width = 595
    Height = 299
    Caption = 'Retornos da Impressora'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    object MStatus: TMemo
      Left = 2
      Top = 15
      Width = 591
      Height = 282
      Align = alClient
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Courier New'
      Font.Style = [fsBold]
      ParentFont = False
      ScrollBars = ssVertical
      TabOrder = 0
      OnChange = MStatusChange
    end
    object PImprimindo: TPanel
      Left = 90
      Top = 128
      Width = 393
      Height = 41
      BevelInner = bvLowered
      Caption = 'Imprimindo. Aguarde um Momento!'
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 1
      Visible = False
    end
  end
  object SB: TStatusBar
    Left = 0
    Top = 336
    Width = 600
    Height = 19
    Panels = <
      item
        Width = 50
      end>
  end
  object MainMenu1: TMainMenu
    OwnerDraw = True
    Left = 40
    Top = 24
    object Programaes1: TMenuItem
      Caption = 'Programa'#231#245'es e Configura'#231#245'es'
      object Arredondamento1: TMenuItem
        Caption = 'Arredondamento'
        OnClick = Arredondamento1Click
      end
      object runcamento1: TMenuItem
        Caption = 'Truncamento'
        OnClick = runcamento1Click
      end
      object HorriodeVero1: TMenuItem
        Caption = 'Hor'#225'rio de Ver'#227'o'
        OnClick = HorriodeVero1Click
      end
      object Alquotas1: TMenuItem
        Caption = 'Al'#237'quotas'
        OnClick = Alquotas1Click
      end
      object FormadePagamento1: TMenuItem
        Caption = 'Forma de Pagamento'
        OnClick = FormadePagamento1Click
      end
    end
    object InformaeseStatus1: TMenuItem
      Caption = 'Informa'#231#245'es e Status'
      object ClicheProprietrio1: TMenuItem
        Caption = 'Clich'#234' Propriet'#225'rio'
        OnClick = ClicheProprietrio1Click
      end
      object CGCIe1: TMenuItem
        Caption = 'C.N.P.J / I.E'
        OnClick = CGCIe1Click
      end
      object AlquotasCadastradas1: TMenuItem
        Caption = 'Al'#237'quotas Cadastradas'
        OnClick = AlquotasCadastradas1Click
      end
      object FormasdePagamentos1: TMenuItem
        Caption = 'Formas de Pagamentos'
        OnClick = FormasdePagamentos1Click
      end
      object EstadodaImpressora1: TMenuItem
        Caption = 'Estado da Impressora'
        OnClick = EstadodaImpressora1Click
      end
      object SubTotal1: TMenuItem
        Caption = 'Sub Total'
        OnClick = SubTotal1Click
      end
      object GTGrandeTotal1: TMenuItem
        Caption = 'G.T - Grande Total'
        OnClick = GTGrandeTotal1Click
      end
      object VendaBrutaDiria1: TMenuItem
        Caption = 'Venda Bruta Di'#225'ria'
        OnClick = VendaBrutaDiria1Click
      end
      object Descontos1: TMenuItem
        Caption = 'Descontos'
        OnClick = Descontos1Click
      end
      object Cancelamentos1: TMenuItem
        Caption = 'Cancelamentos'
        OnClick = Cancelamentos1Click
      end
      object CuponsCancelados1: TMenuItem
        Caption = 'Cupons Cancelados'
        OnClick = CuponsCancelados1Click
      end
      object MapaResumo1: TMenuItem
        Caption = 'Mapa Resumo'
        OnClick = MapaResumo1Click
      end
      object VersoFirmware1: TMenuItem
        Caption = 'Vers'#227'o Firmware'
        OnClick = VersoFirmware1Click
      end
      object VersodaDLL1: TMenuItem
        Caption = 'Vers'#227'o da DLL'
        OnClick = VersodaDLL1Click
      end
      object NumeroCupomFiscal1: TMenuItem
        Caption = 'Numero Cupom Fiscal'
        OnClick = NumeroCupomFiscal1Click
      end
      object ContadordeComprovantesdeCrdito1: TMenuItem
        Caption = 'Numero de Opera'#231#245'es N'#227'o Fiscais'
        OnClick = ContadordeComprovantesdeCrdito1Click
      end
      object NmerodeRedues1: TMenuItem
        Caption = 'N'#250'mero de Redu'#231#245'es'
        OnClick = NmerodeRedues1Click
      end
      object DataeHoraImpressora1: TMenuItem
        Caption = 'Data e Hora Impressora'
        OnClick = DataeHoraImpressora1Click
      end
      object DatadaUltimaReduo1: TMenuItem
        Caption = 'Data da Ultima Redu'#231#227'o'
        OnClick = DatadaUltimaReduo1Click
      end
      object DatadoMovimento1: TMenuItem
        Caption = 'Data do Movimento'
        OnClick = DatadoMovimento1Click
      end
      object MarcamodeloeTipodaImpressora1: TMenuItem
        Caption = 'N'#250'm. de S'#233'rie, N'#250'm. Caixa, Marca, modelo e Tipo da Impressora'
        OnClick = MarcamodeloeTipodaImpressora1Click
      end
    end
  end
  object XPMenu1: TXPMenu
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMenuText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Color = clBtnFace
    IconBackColor = clBtnFace
    MenuBarColor = clBtnFace
    SelectColor = clHighlight
    SelectBorderColor = clHighlight
    SelectFontColor = clMenuText
    DisabledColor = clInactiveCaption
    SeparatorColor = clBtnFace
    CheckedColor = clHighlight
    IconWidth = 24
    DrawSelect = True
    UseSystemColors = True
    OverrideOwnerDraw = False
    Gradient = False
    FlatMenu = False
    Active = True
    Left = 8
    Top = 24
  end
end
